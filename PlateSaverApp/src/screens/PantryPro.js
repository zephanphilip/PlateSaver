...
const PantryPro = () => {
  ...
  useEffect(() => {
    if (editItem) {
      // Parse the quantity string to get quantity and unit separately
      const { quantity, unit } = parseQuantityString(editItem.quantity || '');
...
const styles = StyleSheet.create({
  // Custom quantity and unit styles with platform-specific adjustments
  quantityContainer: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 16,
  ...
  pickerItem: {
    // iOS specific styles for picker items
    height: 120,
    fontSize: 16,
  },
