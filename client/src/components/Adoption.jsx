...
import React, { useState, useEffect } from "react";
import { Box, Card, CardContent, CardHeader, Typography, Button, Input, Alert, CircularProgress, Slider, FormControl, InputLabel, Select, MenuItem } from "@mui/material";
import { MapPin, Clock, Info, X, Loader2, Navigation } from "lucide-react";
import { useUser } from "@clerk/clerk-react";
import General from "../constants/General";
...
const Adoption = () => {
  ...
  const handleAccept = async (donationId) => {
    ...
    try {
      ...
      if (formData.requestedQuantity <= 0 || formData.requestedQuantity > donation.quantity) {
        setError("Invalid quantity requested");
        setLoadingConfirmation(false);
        return;
      }
      ...
      if (response.ok) {
        // Send email notification to the donor
        const donorMessage = `Your donation of ${donation.foodDetails} (${formData.requestedQuantity} of ${donation.quantity}) has been accepted by ${formData.name}. 
          They can be reached at ${formData.phoneNumber}.`;
        ...
        // Send confirmation email to the recipient with location details
        const recipientMessage = `You have successfully accepted the donation of ${donation.foodDetails} (${formData.requestedQuantity} of ${donation.quantity}). 
          Please collect it before ${new Date(donation.expiryDate).toLocaleDateString()}.`;
        ...
        await sendEmailNotification(
          ...
          recipientMessage,
          {
            ...
            location: {
              ...
              address: donation.location.address || "Location coordinates provided"
            },
            acceptedAt: new Date().toISOString(),
            requestedQuantity: formData.requestedQuantity,
            totalQuantity: donation.quantity
  ...
  const handleQuantityChange = (e, donationId) => {
    const value = parseInt(e.target.value, 10);
    const donation = donations.find(d => d._id === donationId);
    
    if (value <= 0 || value > donation.quantity) {
      setError(`Please enter a quantity between 1 and ${donation.quantity}`);
      return;
    }
              ...
              <CardContent>
                <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  ...
                  <Typography variant="body2" sx={{ color: "#FF6B6B", fontWeight: "bold" }}>
                    Available Quantity: {donation.remainingQuantity}
                  </Typography>
                  ...
                  {selectedDonation === donation._id ? (
                    <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
                      ...
                      <FormControl fullWidth>
                        <InputLabel sx={{ color: "#FF6B6B" }}>Quantity</InputLabel>
                        <Select
                          value={formData.requestedQuantity}
                          onChange={(e) => handleQuantityChange(e, donation._id)}
                          label="Quantity"
                          sx={{
                            color: "#FF6B6B",
                        ...
                        >
                          {Array.from({ length: donation.remainingQuantity }, (_, i) => i + 1).map((num) => (
                      ...
                      {loadingConfirmation ? (
                        <CircularProgress sx={{ color: "#FF6B6B", alignSelf: "center", my: 1 }}/>
                      ) : (
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setFormData({
                          ...formData,
                          requestedQuantity: 1 // Reset to 1 when selecting a new donation
                        });
                      }}
                    >
                      Accept Donation
